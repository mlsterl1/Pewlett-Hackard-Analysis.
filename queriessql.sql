...
SELECT * FROM  technical_analysis_one;

---ShowSum of current employess eligible for retimrent
SELECT title,
	COUNT (DISTINCT emp_no)
INTO technical_analysis_one_a
FROM technical_analysis_one
GROUP BY title
;
...
FROM
 ...
 (PARTITION BY (emp_no)
 ORDER BY from_date DESC) rn
 FROM technical_analysis_2
 ) tmp WHERE rn = 1
ORDER BY emp_no
;

SELECT * FROM  technical_analysis_two;
